...
// Luhn's  Algorithm- ადგენს ვალიდურია თუ არა კრედიტ-კარტის ნომერი
// https://cs50.harvard.edu/x/psets/1/credit/   ამ ლინკზე არის აღწერილობა
...
function luhnAlgorithm(creditCardNumber) {
  ...
  while (creditCardNumber > 0) {
    let digit = Math.floor((creditCardNumber % 100) / 10); // ამით ვიღებთ ბოლოდან მეორე რიცხვს
    let product = digit * 2;
    ...
    const remainingDigit = creditCardNumber % 10; // ამით ვიღებთ დარჩენილ რიცხვებს,
    // ანუ ბოლოს, ბოლოდან მესამეს და ა.შ.
...
console.time("luhnAlgorithm"); //რა დრო დაჭირდება შესრულებას მაგას ვზომავ
